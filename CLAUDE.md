...
> `deploy-staging.yml` on `vs*` (in carkedit-online, carkedit-api, carkedit-deploy), so pushing one ships
> *immediately*. They are **not** version markers: the `package.json` bump above is the version record.
> Staging/production deploys happen **only** via the Promote Build buttons on the deploy dashboard
> (dev→staging, staging→prod), which create tags through the GitHub API. This **overrides** the global
> versioning standard's "tag every minor/major bump with `v` and push immediately" — that rule does **not**
> apply to carkedit. A `pre-push` hook (`.githooks/pre-push`) enforces this in each repo; override only with
> `git push --no-verify` if you truly intend to deploy.
